...
export class PnlService {
  ...
  >(
    requests: T,
  ): Promise<{ [K in keyof T]: { period: string; value: number }[] }> {
  ...
  async getNewDatas(period: string) {
    ...
    const EASYBOOK_GROUP_IDS = [19, 17, 26];
    ...
    const getProjectData = async (groupIds: number[]) => {
      ...
      return {
        ...
        shipped: sumDeliveries(latestDeliveries),
        deliveries: latestDeliveries,
        shippedDealIds: Array.from(
          new Set(latestDeliveries.map((delivery) => delivery.dealId)),
        ),
      };
    };
